...
export const EmbeddingSpace: React.FC = () => {
  ...
  const [resultWord, setResultWord] = useState<string>('Queen');
  const [resultCoord, setResultCoord] = useState<{ x: number; y: number }>({ x: 1.15, y: 0.3 });
  ...
  const calculateVectorMath = () => {
    ...
    words.forEach(w => {
      ...
      if (w.word === wordA || w.word === wordB || w.word === wordC) return;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 p-4">
      {/* Arithmetic calculator controls */}
      <div className="md:col-span-5 bg-[#F4EFE6] border border-[#E5DDD0] p-5 rounded-2xl flex flex-col justify-between">
        <div>
          <h4 className="text-[#2E251E] font-bold text-base mb-1 tracking-wide flex items-center gap-2">
            <Compass className="w-5 h-5 text-[#B6532B]" /> Vector Arithmetic
          </h4>
          <p className="text-[#6E6257] text-xs mb-5">
            Embeddings capture conceptual semantics using coordinate geometry.
          </p>

          {/* Algebraic selector inputs */}
          <div className="space-y-4 font-mono text-xs">
            <div>
              <label className="text-[10px] text-[#6E6257] block mb-1.5 uppercase font-semibold">Word A (Base Concept)</label>
              <select
                value={wordA}
                onChange={(e) => setWordA(e.target.value)}
                className="w-full bg-[#FAF6EE] border border-[#E5DDD0] rounded-xl px-3 py-2 text-[#2E251E] focus:outline-none focus:border-[#B6532B] font-mono"
              >
                {words.map(w => (
            ...
            <div className="flex items-center gap-2">
              <span className="text-[#C18C3B] font-bold text-lg px-1">-</span>
              <div className="flex-1">
                <label className="text-[10px] text-[#6E6257] block mb-1.5 uppercase font-semibold">Word B (Subtract Attribute)</label>
                <select
                  value={wordB}
            ...
            <div className="flex items-center gap-2">
              <span className="text-[#3B7A57] font-bold text-lg px-1">+</span>
              <div className="flex-1">
                <label className="text-[10px] text-[#6E6257] block mb-1.5 uppercase font-semibold">Word C (Add Attribute)</label>
                <select
                  value={wordC}
        ...
        <div className="pt-5 border-t border-[#E5DDD0] space-y-3.5">
          <div className="p-3.5 bg-[#FAF6EE] border border-[#E5DDD0] rounded-xl space-y-2 text-center">
            <span className="text-[9px] text-[#6E6257] font-mono uppercase tracking-wider block">Nearest Semantic Match</span>
            <span className="text-xl font-bold font-mono text-[#3B7A57] flex items-center justify-center gap-1.5">
              = {resultWord}
            </span>
          </div>
          ...
          <button
            ...
            className="w-full py-2.5 rounded-xl border border-[#E5DDD0] bg-[#FAF6EE] hover:bg-[#FAF6EE]/80 text-[#6E6257] hover:text-[#2E251E] transition-colors flex items-center justify-center gap-2 text-xs font-semibold cursor-pointer font-mono"
          >
            <RotateCcw className="w-4 h-4" /> Reset Equation
          </button>
        </div>
      </div>
      ...
      <div className="md:col-span-7 flex flex-col items-center">
        <div className="bg-[#FAF6EE] border border-[#E5DDD0] p-4 rounded-2xl w-full flex flex-col justify-center relative shadow-inner">
          <div className="relative overflow-hidden flex justify-center">
            <canvas ref={canvasRef} className="rounded-xl border border-[#E5DDD0] w-full max-w-[400px] h-[320px]" />
          </div>
          ...
          <div className="absolute top-7 left-7 bg-[#2E251E]/90 px-2.5 py-1 rounded-lg border border-[#E5DDD0]/20 text-[9px] font-mono text-[#FAF6EE] uppercase tracking-wide">
            2D Latent Vector Projection (PCA)
          </div>

          <div className="mt-3 p-3 bg-[#F4EFE6] border border-[#E5DDD0] rounded-xl text-[10px] font-mono text-[#6E6257] flex gap-2">
            <HelpCircle className="w-4 h-4 text-[#B6532B] shrink-0" />
            <span>
              Vectors are color-coded: <span className="text-[#B6532B] font-semibold">Royalty terms</span> vs. <span className="text-[#C18C3B] font-semibold">Coding tech terms</span>. Notice how subtraction shifts opposite and addition jumps clusters correctly!
            </span>
          </div>
        </div>
      </div>
