...
export const KMeansSandbox: React.FC = () => {
  ...
  const executeStep = () => {
    ...
    if (step === 'assign') {
      ...
      const updatedCentroids = centroids.map((c, idx) => {
        ...
        if (clusterPoints.length === 0) return c; // keep same
      ...
      centroids.forEach((c, idx) => {
        const dist = Math.sqrt(
          (c.x - updatedCentroids[idx].x) ** 2 + (c.y - updatedCentroids[idx].y) ** 2
        );
        if (dist > 1.0) shifted = true;
      });
      ...
      <div className="md:col-span-5 bg-[#F4EFE6] border border-[#E5DDD0] p-5 rounded-2xl flex flex-col justify-between">
        <div>
          <h4 className="text-[#2E251E] font-bold text-base mb-1 tracking-wide flex items-center gap-2">
            <GitPullRequest className="w-5 h-5 text-[#B6532B]" /> K-Means Sandbox
          </h4>
          <p className="text-[#6E6257] text-xs mb-5">
            Watch clusters organize dynamically using Voronoi partitioning space maps.
          </p>
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="flex justify-between text-xs font-mono mb-2">
                <span className="text-[#2E251E]">Centroids Count (K)</span>
                <span className="text-[#B6532B] font-semibold">{k}</span>
              </div>
              <input
                type="range"
                min="2"
            ...
            <div className="p-3.5 bg-[#FAF6EE] border border-[#E5DDD0] rounded-xl space-y-2 text-xs font-mono text-[#2E251E]">
              <div className="flex justify-between">
                <span className="text-[#6E6257]">Active State:</span>
                <span className="uppercase flex items-center gap-1 font-bold">
                  {step === 'assign' ? 'Assign clusters' : 'Recalculate centroids'}
                  <ArrowRight className="w-3.5 h-3.5 text-[#B6532B]" />
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#6E6257]">Iterations:</span>
                <span className="text-[#B6532B] font-bold">{iterations}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-[#6E6257]">Convergence:</span>
                <span className="text-[#2E251E] font-bold">{isPlaying ? 'Running' : 'Converged / Idle'}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="pt-5 border-t border-[#E5DDD0] space-y-3">
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className={`py-3 rounded-xl flex items-center justify-center gap-1.5 text-xs font-semibold shadow-sm transition-all cursor-pointer ${
                isPlaying
            ...
            <button
              onClick={executeStep}
              disabled={isPlaying}
              className="py-3 rounded-xl border border-[#E5DDD0] bg-[#FAF6EE] hover:bg-[#FAF6EE]/80 text-[#2E251E] disabled:opacity-50 transition-all text-xs font-mono font-bold cursor-pointer"
            >
              Single Step
            </button>
          </div>
