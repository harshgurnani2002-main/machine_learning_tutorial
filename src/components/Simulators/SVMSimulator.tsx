      ...
      <div className="md:col-span-4 bg-[#FAF6EE] border border-[#E5DDD0] p-6 rounded-3xl flex flex-col justify-between shadow-xl">
        <div className="space-y-6">
          <h4 className="text-[#2E251E] font-bold text-xl tracking-tight flex items-center gap-3">
            <Target className="w-6 h-6 text-[#C18C3B]" /> Support Vector Machine
          </h4>
          <p className="text-[#6E6257] text-sm leading-relaxed">
            SVM finds the hyperplane that maximizes the margin between classes. Points with dark rings are <span className="font-semibold text-[#C18C3B]">Support Vectors</span> that dictate the boundary.
          </p>
          ...
          <div className="space-y-3">
            <label className="text-xs text-[#6E6257] font-bold uppercase tracking-wider block font-mono">Kernel Type</label>
            <div className="flex bg-[#FAF6EE] border border-[#E5DDD0] p-1 rounded-xl gap-1">
              <button onClick={() => setKernel('linear')} className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${kernel === 'linear' ? 'bg-[#C18C3B] text-white shadow-md' : 'text-[#6E6257] hover:bg-[#F4EFE6]'}`}>Linear</button>
              <button onClick={() => setKernel('rbf')} className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${kernel === 'rbf' ? 'bg-[#C18C3B] text-white shadow-md' : 'text-[#6E6257] hover:bg-[#F4EFE6]'}`}>RBF (Non-linear)</button>
            </div>
          </div>

          <div className="space-y-4">
            <div className="flex justify-between items-center text-sm font-bold">
              <span className="text-[#6E6257]">Regularization (C):</span>
              <span className="text-[#C18C3B] bg-[#C18C3B]/10 px-3 py-1 rounded-full">{cParam.toFixed(1)}</span>
            </div>
            <p className="text-[10px] text-[#6E6257]">High C = Hard margin (strict), Low C = Soft margin (allows errors)</p>
            <input type="range" min="0.1" max="5.0" step="0.1" value={cParam} onChange={(e) => setCParam(parseFloat(e.target.value))} className="w-full h-2 bg-[#E5DDD0] rounded-lg appearance-none cursor-pointer accent-[#C18C3B]" />
            
            {kernel === 'rbf' && (
              <>
                <div className="flex justify-between items-center text-sm font-bold pt-2">
                  <span className="text-[#6E6257]">Gamma (Spread):</span>
                  <span className="text-[#C18C3B] bg-[#C18C3B]/10 px-3 py-1 rounded-full">{gamma.toFixed(3)}</span>
                </div>
                <input type="range" min="0.001" max="0.05" step="0.001" value={gamma} onChange={(e) => setGamma(parseFloat(e.target.value))} className="w-full h-2 bg-[#E5DDD0] rounded-lg appearance-none cursor-pointer accent-[#C18C3B]" />
              </>
            )}
          </div>
        </div>
      ...
      <div className="md:col-span-8 flex flex-col items-center justify-center">
        <div className="bg-[#F4EFE6] border border-[#E5DDD0] p-2 rounded-3xl w-full flex justify-center shadow-md relative overflow-hidden group">
          ...
          <div className="absolute top-6 left-6 bg-[#2E251E]/80 backdrop-blur-md px-4 py-2 rounded-xl border border-[#E5DDD0]/30 text-xs font-mono text-[#FAF6EE] shadow-lg pointer-events-none opacity-0 group-hover:opacity-100 transition-opacity">
            Solid line = Decision Boundary. Faded lines = Margins.
          </div>
        </div>
      </div>
    </div>
