...
interface DatasetConfig {
  ...
  target: string;
  challenge: string;
  columnTypes: { name: string; type: 'numeric' | 'categorical' | 'target' }[];
  sampleRows: Record<string, string | number>[];
}

const titanicData: DatasetConfig = {
...
const housePricesData: DatasetConfig = {
...
const tweetsData: DatasetConfig = {
...
const datasetMap: Record<string, DatasetConfig> = {
...
function formatCell(val: unknown): string {
  if (val === undefined || val === null || (typeof val === 'number' && isNaN(val))) return 'NaN';
...
function getTypeColor(type: 'numeric' | 'categorical' | 'target'): string {
  switch (type) {
...
function getColumnStyle(col: string, colTypes: { name: string; type: string }[]): string {
  const ct = colTypes.find(c => c.name === col);
  if (ct?.type === 'target') return 'text-[#B6532B] font-bold';
  return 'text-[#2E251E]';
}

export const KaggleDataPreview: React.FC = () => {
  ...
  if (!config) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-12 text-center text-[#6E6257]">
        <Database className="w-16 h-16 mb-4 text-[#CFC5B4]" />
        <h3 className="text-xl font-bold text-[#2E251E] mb-2">Dataset Preview</h3>
        <p>Dataset information is not available for this module.</p>
      </div>
    );
  }
  ...
  return (
    <div className="w-full h-full overflow-y-auto p-4 space-y-4">
      <div className="flex items-start gap-3">
        <div className="p-2.5 rounded-lg bg-[#B6532B] bg-opacity-10">
          <Table className="w-5 h-5 text-[#B6532B]" />
        </div>
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-bold text-[#2E251E] truncate">{config.name}</h3>
          <p className="text-sm text-[#6E6257] mt-0.5">{config.source}</p>
        </div>
      </div>

      <div className="flex flex-wrap gap-3">
        <div className="flex-1 min-w-[140px] bg-white rounded-lg border border-[#E5DDD0] p-3">
          <div className="flex items-center gap-1.5 text-xs text-[#6E6257] mb-1">
            <Info className="w-3.5 h-3.5" />
            <span>Challenge</span>
          </div>
          <p className="text-sm text-[#2E251E] leading-snug">{config.challenge}</p>
        </div>
        <div className="flex gap-3 flex-wrap">
          <div className="bg-white rounded-lg border border-[#E5DDD0] p-3 min-w-[100px]">
            <div className="text-xs text-[#6E6257] mb-0.5">Rows</div>
            <div className="text-lg font-bold text-[#2E251E] font-mono">{config.rows.toLocaleString()}</div>
          </div>
          <div className="bg-white rounded-lg border border-[#E5DDD0] p-3 min-w-[100px]">
            <div className="text-xs text-[#6E6257] mb-0.5">Columns</div>
            <div className="text-lg font-bold text-[#2E251E] font-mono">{config.columns}</div>
          </div>
          <div className="bg-white rounded-lg border border-[#E5DDD0] p-3 min-w-[120px]">
            <div className="text-xs text-[#6E6257] mb-0.5">Target</div>
            <div className="text-base font-bold text-[#B6532B] font-mono">{config.target}</div>
          </div>
        </div>
      </div>

      <div className="bg-white rounded-lg border border-[#E5DDD0] overflow-hidden">
        <div className="px-3 py-2 border-b border-[#E5DDD0] bg-[#FAF6EE] flex items-center gap-2">
          <BarChart3 className="w-4 h-4 text-[#6E6257]" />
          <span className="text-sm font-semibold text-[#2E251E]">Column Types</span>
        </div>
        <div className="flex flex-wrap gap-1.5 p-3">
          {config.columnTypes.map(ct => (
            <span
              key={ct.name}
              className={`text-xs px-2 py-0.5 rounded-md border font-mono ${getTypeColor(ct.type)}`}
            >
              {ct.name}
            </span>
          ))}
      ...
      <div className="bg-white rounded-lg border border-[#E5DDD0] overflow-hidden">
        <div className="px-3 py-2 border-b border-[#E5DDD0] bg-[#FAF6EE]">
          <span className="text-sm font-semibold text-[#2E251E]">Data Preview ({config.sampleRows.length} rows)</span>
        </div>
                ...
                <thead>
                  <tr className="bg-[#FAF6EE]">
                    {columns.map(col => (
                      <th
                        key={col}
                        className={`sticky top-0 px-3 py-2.5 text-left text-xs font-semibold uppercase tracking-wider ${getColumnStyle(col, config.columnTypes)}`}
                      >
                        {col}
                      </th>
                    ))}
