...
export const QLearningSimulator: React.FC = () => {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 h-full bg-[#FAF6EE]">
      <div className="lg:col-span-5 bg-[#F4EFE6] border border-[#E5DDD0] p-6 rounded-2xl flex flex-col justify-between shadow-xl">
        <div className="space-y-6">
          <h4 className="text-[#2E251E] font-bold text-xl tracking-tight flex items-center gap-3">
            <Grid className="w-6 h-6 text-[#B6532B]" /> Q-Learning
          </h4>
          <p className="text-[#6E6257] text-sm leading-relaxed">
            Watch the agent learn to navigate the grid. It updates its <span className="font-semibold text-[#B6532B]">Q-Table</span> using the Bellman Equation to find the shortest path to the goal while avoiding obstacles.
          </p>
          
          <div className="space-y-4 bg-[#FAF6EE] p-4 rounded-xl border border-[#E5DDD0]">
            <div>
              <div className="flex justify-between mb-1">
                <label className="text-[10px] text-[#6E6257] font-mono uppercase font-bold">Learning Rate (α)</label>
            ...
            <div>
              <div className="flex justify-between mb-1">
                <label className="text-[10px] text-[#6E6257] font-mono uppercase font-bold">Discount Factor (γ)</label>
                <span className="text-[10px] font-mono font-bold text-[#B6532B]">{discountFactor.toFixed(2)}</span>
              </div>
              <input 
                type="range" min="0" max="0.99" step="0.01" value={discountFactor} 
            ...
            <div>
              <div className="flex justify-between mb-1">
                <label className="text-[10px] text-[#6E6257] font-mono uppercase font-bold">Exploration (ε)</label>
                <span className="text-[10px] font-mono font-bold text-[#B6532B]">{explorationRate.toFixed(2)}</span>
              </div>
              <input 
                type="range" min="0" max="1" step="0.01" value={explorationRate} 
        ...
        <div className="mt-6 space-y-4">
          <div className="bg-[#FAF6EE] border border-[#E5DDD0] rounded-xl p-3 flex justify-between items-center text-sm font-mono text-[#2E251E]">
            <span className="text-[#6E6257] font-bold">EPISODES</span>
            <span className="text-[#B6532B] font-bold text-lg">{epoch}</span>
          </div>
      ...
      <div className="lg:col-span-7 flex items-center justify-center">
        <div className="bg-[#F4EFE6] p-6 rounded-3xl border border-[#E5DDD0] shadow-inner inline-block">
          <div 
          ...
          >
            {qTable.length > 0 && Array.from({ length: GRID_SIZE }).map((_, y) => (
              Array.from({ length: GRID_SIZE }).map((_, x) => {
                ...
                return (
                  <div 
                    ...
                    `}
                    style={{ backgroundColor: (!isGoal && !isObstacle) ? bgColor : undefined }}
                  >
                    {!isGoal && !isObstacle && maxQ > 0.1 && (
                      <span className="text-[8px] font-mono font-bold text-white opacity-80 mix-blend-difference">
                        {maxQ.toFixed(1)}
                      </span>
                    )}
                    {isAgent && (
                      <div className="absolute inset-0 m-auto w-4/5 h-4/5 bg-[#C18C3B] rounded-full shadow-md z-10 border-2 border-white flex items-center justify-center">
                    ...
                    {isObstacle && !isAgent && (
                      <Flame className="w-3/5 h-3/5 text-[#B6532B]" />
                    )}
                  </div>
                );
              })
          ...
          <div className="mt-4 flex justify-between text-xs font-mono text-[#6E6257] font-bold">
            <span className="flex items-center gap-1"><div className="w-4 h-4 bg-[#C18C3B] rounded-full flex items-center justify-center"><User className="w-3 h-3 text-white" /></div> Agent</span>
            <span className="flex items-center gap-1"><Flame className="w-4 h-4 text-[#B6532B]" /> Fire</span>
            <span className="flex items-center gap-1"><div className="w-4 h-4 bg-[#3B7A57] rounded flex items-center justify-center"><Gem className="w-3 h-3 text-white" /></div> Treasure</span>
            <span className="flex items-center gap-1"><div className="w-3 h-3 bg-[#B6532B] opacity-50 rounded"></div> High Q-Value</span>
          </div>
        </div>
      </div>
    </div>
