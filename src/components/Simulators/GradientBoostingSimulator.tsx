      ...
      <div className="md:col-span-4 bg-[#FAF6EE] border border-[#E5DDD0] p-6 rounded-3xl flex flex-col justify-between shadow-xl">
        <div className="space-y-6">
          <h4 className="text-slate-900 font-bold text-xl tracking-tight flex items-center gap-3">
            <FastForward className="w-6 h-6 text-fuchsia-500" /> Gradient Boosting
          </h4>
          <p className="text-slate-500 text-sm leading-relaxed">
            Boosting combines weak learners (decision stumps) sequentially. Each new tree corrects the <span className="font-semibold text-fuchsia-500">residual errors</span> of the previous ensemble.
          </p>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center text-sm font-bold">
              <span className="text-slate-500">Estimators (Stages):</span>
              <span className="text-fuchsia-600 bg-fuchsia-50 px-3 py-1 rounded-full">{numEstimators}</span>
            </div>
            <input type="range" min="1" max="50" step="1" value={numEstimators} onChange={(e) => setNumEstimators(parseInt(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-fuchsia-500" />
            
            <div className="flex justify-between items-center text-sm font-bold pt-2">
              <span className="text-slate-500">Learning Rate:</span>
              <span className="text-fuchsia-600 bg-fuchsia-50 px-3 py-1 rounded-full">{learningRate.toFixed(2)}</span>
            </div>
            <input type="range" min="0.01" max="1.0" step="0.01" value={learningRate} onChange={(e) => setLearningRate(parseFloat(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-fuchsia-500" />
          </div>

          <div className="p-5 bg-slate-50 border border-slate-200 rounded-2xl space-y-3 shadow-inner mt-4">
            <div className="flex justify-between items-center">
              <span className="text-slate-500 text-sm font-medium">Model Accuracy</span>
              <span className="text-fuchsia-600 font-mono font-bold text-lg">{accuracy.toFixed(1)}%</span>
            </div>
          </div>
        </div>
