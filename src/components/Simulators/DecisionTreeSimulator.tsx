      ...
      <div className="md:col-span-4 bg-[#FAF6EE] border border-[#E5DDD0] p-6 rounded-3xl flex flex-col justify-between shadow-xl">
        <div className="space-y-6">
          <h4 className="text-[#2E251E] font-bold text-xl tracking-tight flex items-center gap-3">
            <GitMerge className="w-6 h-6 text-[#3B7A57]" /> Decision Tree
          </h4>
          <p className="text-[#6E6257] text-sm leading-relaxed">
            Trees partition space into pure rectangular regions using recursive orthogonal splits. Watch how <span className="font-semibold text-[#3B7A57]">Max Depth</span> prevents overfitting.
          </p>

          <div className="space-y-3">
            <label className="text-xs text-[#6E6257] font-bold uppercase tracking-wider block font-mono">Class to Plot</label>
            <div className="flex bg-[#FAF6EE] border border-[#E5DDD0] p-1 rounded-xl gap-1">
              <button
                onClick={() => setActiveLabel(0)}
                className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${
                  activeLabel === 0 ? 'bg-[#B6532B] text-white shadow-md' : 'text-[#6E6257] hover:bg-[#F4EFE6]'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all flex items-center justify-center gap-2 ${
                  activeLabel === 1 ? 'bg-[#C18C3B] text-white shadow-md' : 'text-[#6E6257] hover:bg-[#F4EFE6]'
                }`}
              >
          ...
          <div className="space-y-3">
            <label className="text-xs text-[#6E6257] font-bold uppercase tracking-wider block font-mono">Impurity Metric</label>
            <div className="flex bg-[#FAF6EE] border border-[#E5DDD0] p-1 rounded-xl gap-1">
              <button
                onClick={() => setCriterion('gini')}
                className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${
                  criterion === 'gini' ? 'bg-[#3B7A57] text-white shadow-md' : 'text-[#6E6257] hover:bg-[#F4EFE6]'
                }`}
              >
                Gini Impurity
              </button>
              <button
                ...
                className={`flex-1 py-2.5 rounded-lg text-sm font-bold transition-all ${
                  criterion === 'entropy' ? 'bg-[#3B7A57] text-white shadow-md' : 'text-[#6E6257] hover:bg-[#F4EFE6]'
                }`}
              >
                Entropy
              </button>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center text-sm font-bold">
              <span className="text-[#6E6257]">Max Depth Limit:</span>
              <span className="text-[#3B7A57] bg-[#3B7A57]/10 px-3 py-1 rounded-full">{maxDepth}</span>
            </div>
            <input
              type="range"
              min="1"
              ...
              className="w-full h-2 bg-[#E5DDD0] rounded-lg appearance-none cursor-pointer accent-[#3B7A57]"
            />
          </div>

          <div className="p-5 bg-[#FAF6EE] border border-[#E5DDD0] rounded-2xl space-y-3 shadow-inner">
            <div className="flex justify-between items-center">
              <span className="text-[#6E6257] text-sm font-medium">Accuracy</span>
              <span className="text-[#3B7A57] font-mono font-bold text-lg">{accuracy.toFixed(1)}%</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-[#6E6257] text-sm font-medium">Tree Size</span>
              <span className="text-[#2E251E] font-mono font-bold">{totalNodes} nodes</span>
            </div>
          </div>
        </div>
