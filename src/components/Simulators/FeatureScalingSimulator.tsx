...
export const FeatureScalingSimulator: React.FC = () => {
  ...
  const [scalingMethod, setScalingMethod] = useState<'minmax' | 'standard'>('minmax');
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 p-4">
      <div className="md:col-span-4 bg-[#F4EFE6] border border-[#E5DDD0] p-5 rounded-2xl flex flex-col justify-between">
        <div className="space-y-4">
          <h4 className="text-[#2E251E] font-bold text-base tracking-wide flex items-center gap-2">
            <Target className="w-5 h-5 text-[#B6532B]" /> Feature Scaling
          </h4>
          <p className="text-[#6E6257] text-xs">
            Add points on the left to see how they are transformed on the right. 
            Toggle the outlier to see how it squashes the rest of the data.
          </p>
          
          <div className="space-y-2">
            <label className="text-xs text-[#2E251E] font-mono block">Scaling Method</label>
            <div className="flex gap-2">
              <button
                onClick={() => setScalingMethod('minmax')}
          ...
          <label className="flex items-center gap-2 text-sm text-[#2E251E] cursor-pointer mt-4">
            <input 
              type="checkbox" 
              checked={showOutlier} 
              onChange={(e) => setShowOutlier(e.target.checked)}
              className="rounded text-[#B6532B] focus:ring-[#B6532B]"
            />
            <span className="font-mono text-xs">Inject Extreme Outlier</span>
          </label>
          
          <div className="p-3 bg-[#FAF6EE] border border-[#E5DDD0] rounded-xl space-y-2 text-xs font-mono mt-4">
            <div className="flex justify-between">
              <span className="text-[#6E6257]">Data Mean (X, Y):</span>
              <span className="text-[#2E251E] font-bold">({meanX.toFixed(1)}, {meanY.toFixed(1)})</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#6E6257]">Data Std (X, Y):</span>
              <span className="text-[#2E251E] font-bold">({stdX.toFixed(1)}, {stdY.toFixed(1)})</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#6E6257]">Range (X, Y):</span>
              <span className="text-[#2E251E] font-bold">({rangeX.toFixed(1)}, {rangeY.toFixed(1)})</span>
            </div>
          </div>
        </div>

        <button
          onClick={() => { setPoints([]); setShowOutlier(false); }}
          className="w-full py-2.5 mt-4 rounded-xl border border-[#E5DDD0] bg-[#FAF6EE] hover:bg-[#FAF6EE]/80 text-[#6E6257] hover:text-[#2E251E] transition-all flex items-center justify-center gap-2 text-xs font-semibold"
        >
      ...
      <div className="md:col-span-4 flex flex-col items-center">
        <div className="bg-[#FAF6EE] border border-[#E5DDD0] p-4 rounded-2xl relative w-full flex justify-center shadow-inner">
          <canvas
            ref={canvasRef}
            onClick={handleCanvasClick}
            className="rounded-xl border border-[#E5DDD0] cursor-crosshair w-full max-w-[400px] h-[300px]"
          />
      ...
      <div className="md:col-span-4 flex flex-col items-center">
        <div className="bg-[#FAF6EE] border border-[#E5DDD0] p-4 rounded-2xl relative w-full flex justify-center shadow-inner">
          <canvas
            ref={scaledCanvasRef}
            className="rounded-xl border border-[#E5DDD0] w-full max-w-[400px] h-[300px]"
          />
          <div className="absolute top-7 left-7 bg-[#2E251E]/90 px-2.5 py-1 rounded-lg border border-[#E5DDD0]/20 text-[9px] font-mono text-[#FAF6EE] uppercase tracking-wide">
            {scalingMethod === 'minmax' ? 'MinMax Scaled [0, 1]' : 'Standardized (Z-Score)'}
          </div>
        </div>
      </div>
    </div>
